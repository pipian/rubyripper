...
class TC_ScanDiscCdparanoia < Test::Unit::TestCase
	...
	def setup
		...
		@cdpar = ScanDiscCdparanoia.new(@prefs, @fire, @perm)
	end
...
audio disc in first..."), @cdpar.status)
	end
...
@cdpar.status)
		assert_equal(false, @cdpar.get('multipleDriveSupport'))
	end
	...
	def test_DriveUnknown
		...
		@cdpar.scan()
		assert_equal(_('ERROR: drive testDrive is not found'), @cdpar.status)
	end
	...
	def	test_PureAudioDisc
		...
		@cdpar.scan()
		assert_equal(10, @cdpar.get('audiotracks'))
		assert_equal("HL-DT-ST DVDRAM GH22NS40 NL01", @cdpar.get('devicename'))
		assert_equal("36:12", @cdpar.get('playtime'))
		assert_equal(162919, @cdpar.get('totalSectors'))
		assert_equal(_('ok'), @cdpar.status)
	
		startSectors = [0, 13209, 36539, 53497, 68172, 81097, 87182, 106732,
122218, 124080]
		startSectors.each_index do |index|
			assert_equal(startSectors[index], @cdpar.get('startSector')[index + 1])
			assert_equal(startSectors[index], @cdpar.getStartSector(index + 1))
		end

		lengthSectors = [13209, 23330, 16958, 14675, 12925, 6085, 19550, 15486,
1862, 38839]
		lengthSectors.each_index do |index|
			assert_equal(lengthSectors[index], @cdpar.get('lengthSector')[index + 1])
			assert_equal(lengthSectors[index], @cdpar.getLengthSector(index + 1))
		end
...
'02:52.25', '01:21.10', '04:20.50', '03:26.36', '00:24.62', '08:37.64']
		lengthText.each_index do |index|
			assert_equal(lengthText[index], @cdpar.get('lengthText')[index + 1])
			assert_equal(lengthText[index], @cdpar.getLengthText(index + 1))
		end
	end
	...
	def	test_AudioDiscWithDataTrack
		...
		@cdpar.scan()
		assert_equal(12, @cdpar.get('audiotracks'))
		assert_equal("HL-DT-ST DVDRAM GH22NS40 NL01", @cdpar.get('devicename'))
		assert_equal("58:37", @cdpar.get('playtime'))
		assert_equal(263797, @cdpar.get('totalSectors'))
		assert_equal(_('ok'), @cdpar.status)
