...
class TC_FreedbRecordParser < Test::Unit::TestCase
	...
	def test_NormalAudioDisc
		...
		record = FreedbRecordParser.new(file)

		assert_equal(_('ok'), record.status)
		assert_equal('98113b0e', record.metadata['discid'])
		assert_equal('Type O Negative', record.metadata['artist'])
		assert_equal('Bloody Kisses', record.metadata['album'])
		assert_equal('1993', record.metadata['year'])
		assert_equal('Gothic', record.metadata['genre'])
...
13=>'Blood & Fire', 14=>'Can\'t Lose You'}.each do |key, value|
			assert_equal(value, record.metadata['tracklist'][key])
		end
		assert_equal(nil, record.metadata['extraDiscInfo'])
		assert_equal(nil, record.metadata['oldTracklist'])
	end
	...
	def test_AudioDiscExtraInfo
		...
		record = FreedbRecordParser.new(file)
		assert_equal(_('ok'), record.status)
		assert_equal('8d093509', record.metadata['discid'])
		assert_equal('Judas Priest', record.metadata['artist'])
		assert_equal('Sad Wings Of Destiny', record.metadata['album'])
		assert_equal('1976', record.metadata['year'])
		assert_equal('Rock', record.metadata['genre'])
...
bass\\nAlan Mooret- drums", record.metadata['extraDiscInfo'])
		assert_equal(nil, record.metadata['oldTracklist'])
	end

	# test various artist disc with / separator
	def test_VariousDiscSlash
		file = File.read(File.join($localdir, 'data/freedb/disc003'))
		record = FreedbRecordParser.new(file)

		assert_equal(_('ok'), record.status)
		assert_equal('0a0dd914', record.metadata['discid'])
		assert_equal('Various', record.metadata['artist'])
		assert_equal('Années 60 CD1', record.metadata['album'])
		assert_equal('2004', record.metadata['year'])
		assert_equal('Misc', record.metadata['genre'])
...
19=>'BJ THOMAS', 20=>'THE TREMOLOES'}.each do |key, value|
			assert_equal(value, record.metadata['varArtist'][key])
		end
		assert_equal(nil, record.metadata['extraDiscInfo'])
...
20=>'THE TREMOLOES / Suddenly You Love Me'}.each do |key, value|
			assert_equal(value, record.metadata['oldTracklist'][key])
		end		
	end

	#test disc with multiple rows per track and title
	def test_MultipleRows
		file = File.read(File.join($localdir, 'data/freedb/disc004'))
		record = FreedbRecordParser.new(file)

		assert_equal(_('ok'), record.status)
		assert_equal('0a0f5811', record.metadata['discid'])
		assert_equal('Maria Venuti', record.metadata['artist'])
		assert_equal('Maria Venuti Sings Schubert, Schoenberg, Schumann', record.metadata['album'])
		assert_equal('1994', record.metadata['year'])
		assert_equal('Classical', record.metadata['genre'])
...
17=>'Franz Schubert: Delphine Op. 124, No. 1'}.each do |key, value|
	...
	def test_VariousDiscDash
		file = File.read(File.join($localdir, 'data/freedb/disc005'))
		record = FreedbRecordParser.new(file)
		assert_equal(_('ok'), record.status)
		assert_equal('0a105e12', record.metadata['discid'])
		assert_equal('Sampler Klezmer', record.metadata['artist'])
		assert_equal('Doyres (Generations)', record.metadata['album'])
		assert_equal(nil, record.metadata['year'])
		assert_equal('Klezmer', record.metadata['genre'])
...
17=>'Kapelye', 18=>'New Shtetl Band'}.each do |key, value|
	...
	def test_RecordIso88591Encoding
		file = File.read(File.join($localdir, 'data/freedb/disc006'))
		record = FreedbRecordParser.new(file)
		assert_equal(_('ok'), record.status)
		assert_equal('1c11c714', record.metadata['discid'])
		assert_equal('Various', record.metadata['artist'])
		assert_equal('Jazz Manouche CD 1', record.metadata['album'])
		assert_equal(nil, record.metadata['year'])
		assert_equal('Jazz', record.metadata['genre'])
